...
public static class BetterStreamingAssets
{
    ...
    internal static class EditorImpl
    {
        ...
        internal static bool TryGetInfo(string path, out ReadInfo info)
        {
            ...
            {
#if !UNITY_EDITOR
                return LooseFilesImpl.TryGetInfoWeb(path, out info);//TryGetInfo
#else
    ...
    internal static class LooseFilesImpl
    {
        ...
        public static bool TryGetInfoWeb(string path, out ReadInfo info)
        {
            ...
            if (filePath.Contains("://") || filePath.Contains(":///"))
            {
                ...
                Debug.Log("TRY GET INFO WEB true");
                return true;
            }
            Debug.Log("TRY GET INFO WEB false");
            return false;
        }
        private static IEnumerator GetBinaryData(string filePath, ReadInfo info)
        {
            WWW www = new WWW(filePath);
            //result = www.text;
            yield return www;
        ...
        public static bool DirectoryExists(string path)
        {
            ...
            return Directory.Exists(s_root + normalized);//+normalized
        }

        public static byte[] ReadAllBytes(string path)
        {
            ReadInfo info;
            Debug.Log("START READ BYTES IN BSA");
            if ( !TryGetInfo(path, out info) )//TryGetInfoWeb
                ThrowFileNotFound(path);
            //Debug.Log("");
            //возвратить массив байтов
            Debug.Log("READ ALL BYTES BSA " + info.readPath);
            return File.ReadAllBytes(info.readPath);//info.bytes;//File.ReadAllBytes(info.readPath);
        }

        public static System.IO.Stream OpenRead(string path)
        {
            ReadInfo info;
            if ( !TryGetInfoWeb(path, out info) )
                ThrowFileNotFound(path);
    ...
    internal static class ApkImpl
    {
        ...
        public static bool DirectoryExists(string path)
        {
            var normalized = path;//PathUtil.NormalizeRelativePath(path, forceTrailingSlash : true);
        ...
        public static string[] GetFiles(string path, string searchPattern, SearchOption searchOption)
        {
            if ( path == null )
                throw new ArgumentNullException("path");
            Debug.Log("GET FILES IN PATH " + path);
            var actualDirPath = path;//PathUtil.NormalizeRelativePath(path, forceTrailingSlash : true);
